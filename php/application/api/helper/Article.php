...
class Article extends Base
{
    ...
    private function saveData($parame)
    {
        ...
        $saveData['catid']          = isset($parame['catid']) ? $parame['catid'] : 0;
        $saveData['author']         = isset($parame['author']) ? $parame['author'] : '';
        $saveData['content']        = isset($parame['content']) ? $parame['content'] : '';
        $saveData['sort']           = isset($parame['sort']) ? intval($parame['sort']) : 1;
    	...
    	if ($id <= 0) {
            $saveData['sort']                       = isset($parame['sort']) ? intval($parame['sort']) : 1;
            $saveData['status']                     = isset($parame['status']) ? $parame['status'] : 1;
     ...
     * @return [array]         接口输出数据
     */
    private function notice($parame)
    {
        //主表数据库模型
        $dbModel                    = model($this->mainTable);
        ...
        //检索条件 需要对应的模型里面定义查询条件 格式为formatWhere...
        $modelParame['whereFun']    = 'formatWhereNotice';
